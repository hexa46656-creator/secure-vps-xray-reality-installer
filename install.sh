...
INSTALL_LOCK_FILE="/etc/vpsguard/secure-vps-xray-reality-installer.lock"
INSTALL_REPORT_FILE="/root/secure-vps-xray-reality-installer-install-report.txt"
...
repo=secure-vps-xray-reality-installer
service=xray
config=${XRAY_CONFIG}
...
probe_network() {
  ...
  if command -v ping >/dev/null 2>&1; then
    if ping -c 3 -W 2 1.1.1.1 >/tmp/secure-vps-xray-ping.log 2>&1; then
      log "Ping to 1.1.1.1 succeeded."
      awk '/^rtt|^round-trip/ {print}' /tmp/secure-vps-xray-ping.log | tail -n1 || true
    else
      warn "Ping to 1.1.1.1 failed."
    fi
    rm -f /tmp/secure-vps-xray-ping.log
  else
...
Repository: secure-vps-xray-reality-installer
